  ...
  <div v-if="!loading">
    ...
    <div style="background-color: #F0F9F8;padding-bottom: 20px">
      ...
      <div v-for="(item, index) in ValueDetails" :key="index">
        ...
        <div v-if="index % 2 === 0">
          <MoosValueLeft :rightPill="index + 'left'" :leftPill="index + 'right'" :oldWays="item.oldWays"
            :moosWays="item.moosWays" :pillTitle="item.pillTitle" :valuePercentage="item.valuePercentage"
            :valueDes="item.valueDes" :isVisible="isLastValue(index)" :imageURL="item.imageURL" />
...
import { getWareHouseTitle, BusinessGetStart, getWareHousingMoosValues } from "@/services/business.js";


export default {
  components: { Card, GetStart, MapAndContact, MoosValueLeft, MoosValueRight },
  name: "business_solutions2",
